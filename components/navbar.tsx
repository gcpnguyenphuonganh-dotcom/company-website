    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/products?fields[0]=name&fields[1]=slug&fields[2]=category&pagination[pageSize]=100`
        )
        if (!res.ok) return
        ...
        setCategoryGroups(Array.from(map.entries()).map(([name, prods]) => ({ name, products: prods })))
      } catch (err) {
        console.error(err)
      }
    }
  ...
  return (
    ...
    >
      ...
      <Link href={`/${lang}`} className="flex items-center gap-2 z-10">
        <img src="/logo.png" alt="Company Logo" className="w-auto transition-all duration-300"
