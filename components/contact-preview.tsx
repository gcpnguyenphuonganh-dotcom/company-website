...
export default function ContactPreview() {
  ...
  const cardInner = (info: typeof contactInfo[0], index: number) => (
    <>
      <div className={`w-12 h-12 rounded-xl bg-[#020c1a]/5 flex items-center justify-center mb-4 transition-all duration-500 ${hoveredIndex === index ? "bg-[#020c1a] scale-110" : ""}`}>
        <info.icon className={`w-6 h-6 transition-colors duration-500 ${hoveredIndex === index ? "text-white" : "text-[#020c1a]"}`} />
      </div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-1 bg-[#020c1a] rounded-b-2xl transition-all duration-500 ${hoveredIndex === index ? "opacity-100" : "opacity-0"}`} />
      {(info.key === "visit" || info.key === "email" || info.key === "call") && (
        <div className={`absolute top-3 right-3 transition-all duration-300 ${hoveredIndex === index ? "opacity-100" : "opacity-0"}`}>
      ...
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <p className={`text-lg text-muted-foreground leading-relaxed transition-all duration-700 delay-200 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
